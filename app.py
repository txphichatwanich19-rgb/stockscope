...
st.markdown(
    """
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Anuphan:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600;700&display=swap');
    ...
    p, span, div, h1, h2, h3, h4, h5, h6, label {
        font-family: 'Inter', 'Anuphan', system-ui, -apple-system, sans-serif !important;
    ...
    /* Global typography — light on dark */
    body, .stMarkdown p, label { color: #e0e9ff; }
    h1, h2, h3, h4, h5, h6, .stMarkdown h1, .stMarkdown h2, .stMarkdown h3 {
        color: #f0f7ff;
        letter-spacing: -0.025em;
        font-weight: 700;
    }
    .stCaption, [data-testid="stCaptionContainer"] { color: #94a8d4 !important; }
    ...
    .hero:hover { border-color: #2a3244; }
    .hero::before {
        content: '';
        position: absolute; top: 0; left: 0; right: 0; height: 1px;
        background: linear-gradient(90deg, transparent 0%, rgba(190,242,100,0.6) 50%, transparent 100%);
    }
    .hero:hover {
        border-color: rgba(190,242,100,0.35);
        box-shadow:
            0 1px 2px rgba(0,0,0,0.4),
            0 28px 64px -10px rgba(59,130,246,0.28),
            0 0 100px rgba(190,242,100,0.1);
    }
    .hero .sym {
        display: inline-block;
    ...
    .session-open   { background: rgba(190,242,100,0.12); color: #d9f99d; border-color: rgba(190,242,100,0.35); }
    .session-pre    { background: rgba(251,191,36,0.12); color: #fde68a; border-color: rgba(251,191,36,0.35); }
    .session-post   { background: rgba(196,181,253,0.12); color: #c4b5fd; border-color: rgba(196,181,253,0.35); }
    .session-closed { background: rgba(148,168,212,0.12); color: #cbd5e1; border-color: rgba(148,168,212,0.3); }

    .ext-hours {
        margin-top: 0.85rem;
        padding-top: 0.85rem;
        border-top: 1px dashed rgba(190,242,100,0.18);
        text-align: right;
    }
    .ext-hours .ext-head {
        font-size: 0.72rem;
    ...
    .news-card:hover { border-color: #2a3244; }
    .news-card .title {
        font-family: 'Inter', 'Anuphan', sans-serif !important;
        color: #f1f5f9;
        font-weight: 600;
    ...
    .news-card .title a { color: #f1f5f9; text-decoration: none; }
    .news-card .title a:hover { color: #3b82f6; }
    .news-card .orig   { color: #64748b; font-size: 0.78rem; margin-top: 0.3rem; font-style: italic; }
    .news-card .meta   { color: #64748b; font-size: 0.74rem; margin-top: 0.45rem; }
    .news-card .summary{ color: #94a3b8; font-size: 0.88rem; margin-top: 0.6rem; line-height: 1.6; }

    /* Verdict card — pro flat */
    .verdict {
        border-radius: 6px;
    ...
    .verdict.bull { color: #10b981; border-left-color: #10b981; }
    ...
    .macro-tile:hover { border-color: #2a3244; }
    .macro-label {
    ...
    .macro-chip.up   { background: #052e2b; color: #10b981; }
    .macro-chip.down { background: #2e0512; color: #ef4444; }
    ...
    .mover-cell:hover { border-color: #2a3244; }
    .mover-cell.mover-up   { border-left: 3px solid #10b981; }
    .mover-cell.mover-down { border-left: 3px solid #ef4444; }
    .mover-sym {
        font-family: 'JetBrains Mono', monospace;
    ...
    .mover-up .mover-pct   { color: #10b981; }
    .mover-down .mover-pct { color: #ef4444; }
    .mover-price {
        font-family: 'JetBrains Mono', monospace;
        font-size: 0.76rem;
        color: #64748b;
    ...
    .sector-cell:hover { border-color: #2a3244; }
    .sector-sym {
        font-family: 'JetBrains Mono', monospace;
        font-weight: 600;
        font-size: 0.85rem;
    ...
    .pos52-title {
        ...
        color: #94a8d4; letter-spacing: 0.12em;
        text-transform: uppercase;
    }
    .pos52-pct {
        font-family: 'JetBrains Mono', monospace;
    ...
    .pos52-track {
        ...
        background: linear-gradient(90deg, rgba(251,113,133,0.5) 0%, rgba(148,168,212,0.3) 50%, rgba(190,242,100,0.6) 100%);
    ...
    .pos52-ends {
        display: flex; justify-content: space-between;
        margin-top: 0.55rem; font-size: 0.78rem; color: #94a8d4;
        font-family: 'JetBrains Mono', monospace;
        font-variant-numeric: tabular-nums;
    }
    ...
    .mini-chip.up   { background: #052e2b; color: #10b981; border-color: #10b98155; }
    .mini-chip.down { background: #2e0512; color: #ef4444; border-color: #ef444455; }
    .mini-chip.flat { background: #1a2233; color: #94a3b8; border-color: #2a3244; }
    ...
    .level-tile.entry     { border-left-color: #10b981; }
    .level-tile.stop      { border-left-color: #f59e0b; }
    .level-tile.resistance { border-left-color: #ef4444; }
    .level-tile.target    { border-left-color: #3b82f6; }
    .level-tile.current   { border-left-color: #64748b; }
    .level-tile:hover { transform: translateY(-2px); border-color: rgba(190,242,100,0.3); }
    .level-tile .role {
    ...
    .level-tile.stop .price      { color: #fdba74; text-shadow: 0 0 12px rgba(251,146,60,0.4); }
    .level-tile.target    { border-color: rgba(59,130,246,0.35); box-shadow: 0 0 20px rgba(59,130,246,0.06); }
    .level-tile.target .price    { color: #93c5fd; text-shadow: 0 0 12px rgba(59,130,246,0.4); }
    .level-tile.resistance { border-color: rgba(251,113,133,0.35); box-shadow: 0 0 20px rgba(251,113,133,0.06); }
    .level-tile.resistance .price { color: #fda4af; text-shadow: 0 0 12px rgba(251,113,133,0.4); }
    .level-tile.current   {
        background: linear-gradient(180deg, rgba(59,130,246,0.1) 0%, rgba(14,19,57,0.7) 100%);
        border-color: rgba(59,130,246,0.35);
    }
    .level-tile.current .price   { color: #f0f7ff; text-shadow: 0 0 16px rgba(190,242,100,0.4); }

    .rr-badge {
        display: inline-block;
        background: linear-gradient(135deg, rgba(190,242,100,0.12) 0%, rgba(59,130,246,0.08) 100%);
    ...
    .rr-badge b { color: #bef264; font-weight: 800; }

    .pick-intro {
        color: #c8d3ef; font-size: 0.9rem; margin-bottom: 0.8rem;
    }
    .pick-disclaim {
        background: linear-gradient(180deg, rgba(251,191,36,0.08) 0%, rgba(14,19,57,0.7) 100%);
        border: 1px solid rgba(251,191,36,0.3);
        border-radius: 12px; padding: 0.75rem 0.95rem;
        color: #fde68a; font-size: 0.85rem; margin-bottom: 1rem;
    }
    ...
    .lv-row-up .lv-arrow { color: #fda4af; }
    .lv-row-down .lv-arrow { color: #d9f99d; }
    .lv-pills { display: flex; gap: 0.45rem; flex-wrap: wrap; }
    .lv-empty { font-size: 0.82rem; color: #6b7aa8; font-style: italic; }
    ...
    .lv-pill.lv-sell { background: #2e0512; color: #ef4444; border-color: #ef444455; }
    .lv-pill.lv-buy  { background: #052e2b; color: #10b981; border-color: #10b98155; }
    .lv-pill.lv-stop { background: #2e1a05; color: #f59e0b; border-color: #f59e0b55; }
    .lv-pill .lv-status {
        font-family: 'JetBrains Mono', monospace;
        font-size: 0.68rem;
        font-weight: 600;
    ...
    .lv-now {
        ...
        margin: 0.5rem 0;
        background: linear-gradient(90deg, rgba(190,242,100,0.06) 0%, rgba(59,130,246,0.08) 50%, rgba(190,242,100,0.06) 100%);
        border-top: 1px solid rgba(190,242,100,0.2);
        border-bottom: 1px solid rgba(190,242,100,0.2);
        border-radius: 10px;
    }
    .lv-now-label {
        font-size: 0.78rem;
    ...
    .lv-now-price {
        ...
        letter-spacing: -0.015em;
        font-variant-numeric: tabular-nums;
        text-shadow: 0 0 16px rgba(190,242,100,0.3);
    }
    .lv-now-tag {
        font-size: 0.7rem;
        font-weight: 800;
...
def zone_status(df: pd.DataFrame, zone_price: float, kind: str, lookback: int = 30, tol: float = 0.008) -> dict:
    ...
    if df.empty or len(df) < 2:
        return {"key": "unknown", "label": "—", "days_ago": None, "color": "#64748b"}
    ...
    if kind == "support":
        ...
        if broken_idx is not None:
            ...
            return {"key": "broken", "label": f"⬇ BROKEN · {days}d" if days > 0 else "⬇ BROKEN", "days_ago": days, "color": "#ef4444"}
        if touched_idx is not None:
            days = len(recent) - 1 - touched_idx
            return {"key": "touched", "label": f"◉ TESTED · {days}d" if days > 0 else "◉ TESTED", "days_ago": days, "color": "#10b981"}
        # not yet touched
        if current < zone_price * 1.03:
            return {"key": "approaching", "label": "● APPROACHING", "days_ago": None, "color": "#f59e0b"}
        return {"key": "waiting", "label": "○ PENDING", "days_ago": None, "color": "#64748b"}
        ...
        if broken_idx is not None:
            days = len(recent) - 1 - broken_idx
            return {"key": "broken", "label": f"⬆ BROKEN · {days}d" if days > 0 else "⬆ BROKEN", "days_ago": days, "color": "#10b981"}
        if touched_idx is not None:
            days = len(recent) - 1 - touched_idx
            return {"key": "touched", "label": f"◉ TESTED · {days}d" if days > 0 else "◉ TESTED", "days_ago": days, "color": "#ef4444"}
...
if st.session_state.page == "💼 พอร์ตของฉัน":
    ...
    with screenshot_tab:
        st.caption(
            ...
            "· ไม่มีการวิเคราะห์อัตโนมัติ (ให้กรอกที่ tab '➕ เพิ่ม/แก้ไข' หรือ '📄 นำเข้า CSV' เอง)"
        )
...
if st.session_state.page == "🌐 ภาพรวมตลาด":
    ...
    if sectors:
        ...
        def _sec_color(pct: float) -> tuple[str, str, str]:
            # (left-border, bg, text) — professional palette
            if pct >= 2:    return "#10b981", "#141a26", "#10b981"
            if pct >= 1:    return "#10b981", "#141a26", "#34d399"
            if pct >= 0.3:  return "#059669", "#141a26", "#34d399"
            if pct > -0.3:  return "#334155", "#141a26", "#94a3b8"
            if pct > -1:    return "#dc2626", "#141a26", "#f87171"
            if pct > -2:    return "#dc2626", "#141a26", "#f87171"
            return "#ef4444", "#141a26", "#ef4444"

        st.caption("คลิกชื่อ ETF เพื่อดูรายละเอียดเต็ม · สี = % เปลี่ยนแปลงวันนี้")
        hm_html = '<div class="sector-grid">'
        for s in sectors_sorted:
            border, bg, txt = _sec_color(s["d1"])
            hm_html += (
                f'<div class="sector-cell" style="background:{bg};border-left-color:{border};">'
                f'<div class="sector-sym">{s["sym"]}</div>'
                f'<div class="sector-name">{s["name"]}</div>'
                f'<div class="sector-pct" style="color:{txt};">{s["d1"]:+.2f}%</div>'
        ...
        try:
            ...
            if available:
                ...
                fig_q.update_layout(
                    ...
                    font=dict(family="Inter, sans-serif", color="#94a8d4", size=11),
                    legend=dict(orientation="h", yanchor="bottom", y=1.02,
                                xanchor="right", x=1, bgcolor="rgba(0,0,0,0)"),
                )
                fig_q.update_xaxes(gridcolor="rgba(190,242,100,0.06)", color="#94a8d4")
                fig_q.update_yaxes(gridcolor="rgba(190,242,100,0.06)", color="#94a8d4", zerolinecolor="rgba(190,242,100,0.1)")
